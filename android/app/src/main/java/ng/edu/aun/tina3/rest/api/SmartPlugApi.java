...
public class SmartPlugApi extends Api {
    ...
    public static void gets(@NotNull final DoubleReceiver<SmartPlug.SmartPlugList, LitigyException> callbackReceiver){
        ...
        try {
            Authenticator.getInstance().getAccessToken(new DoubleReceiver<String, LitigyException>() {
                ...
                public void onReceive1(String s) {
                    try {
                        TINA3Request.<SmartPlug.SmartPlugList>withAuthorization(s).withEndpoint(buildEndpoint(PATH, method))
                                .withCallbackReceiver(callbackReceiver).GET(SmartPlug.SmartPlugList.class);
                    } catch (IOException e) {
                        callbackReceiver.onReceive2(LitigyException.consumeIOException(e));
                    }
                }
