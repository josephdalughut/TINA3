...
public class LoginFragment extends Fragtivity implements DoubleReceiver<User, LitigyException> {
    ...
    private void onLogin(){
        progressDialog = ProgressDialog.getInstance(getString(R.string.please_wait), getString(R.string.logging_in), getColor(R.color.tina_green), false);
        progressDialog.show(getChildFragmentManager(), null);
        AuthApi.login(id.getText().toString(), password.getText().toString(), this);
    }
        ...
        try{
            task = Authenticator.getInstance().setUserAsync(user, new DoubleReceiver<User, LitigyException>() {
                        ...
                        public void onReceive2(LitigyException e) {
                            progressDialog.dismissAllowingStateLoss();
                            Snackbar.showLong(LoginFragment.this, R.string.error_service_unavailable);
                        }
                    });
        ...
        try{
            ...
            switch (e.toServiceException()){
                ...
                case NotFoundException:
                    Snackbar.showLong(this, R.string.error_login_not_found);
                    break;
                case ConflictException:
                    Snackbar.showLong(this, R.string.error_login_conflict);
                    break;
                default:
